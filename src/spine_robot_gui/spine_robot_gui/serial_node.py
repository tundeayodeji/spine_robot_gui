...
class SerialNode(Node):
    ...
    def timer_callback(self):
        ...
        if decoded_info:
            ...
            # Updates displayed number of cycles completed for Forward and Backward Drive GUI tabs
            try:
                int(decoded_info)
                
                if self.ui.p3_LblTaskRunningUpdate.isVisible():
                    cycles = int(self.ui.p3_CyclesSpinBox.value())
                    self.ui.p3_LblTaskRunningUpdate.setText(f"{decoded_info} of {cycles} cycles complete")

                elif self.ui.p4_LblTaskRunningUpdate.isVisible():
                    cycles = int(self.ui.p4_CyclesSpinBox.value())
                    self.ui.p4_LblTaskRunningUpdate.setText(f"{decoded_info} of {cycles} cycles complete")
